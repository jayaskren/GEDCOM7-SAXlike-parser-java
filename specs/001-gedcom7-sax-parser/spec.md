...
**Feature Branch**: `001-gedcom7-sax-parser`
**Created**: 2026-03-04
**Status**: Draft
...
- Q: Does HEAD emit startRecord/endRecord events like other
  records, or is it consumed silently? → A: HEAD emits
  events like any other record (startRecord/endRecord plus
  substructure events). startDocument fires before HEAD's
  startRecord. Additionally, startDocument carries a
  GedcomHeaderInfo object with standard HEAD metadata
  (version, source system, etc.) for quick access, while
  still allowing consumers to parse custom/extension tags
  in HEAD via the normal event stream.
- Q: Is GedcomReader thread-safe, or single-threaded per
  instance? → A: Single-threaded per instance. No
  synchronization overhead. Callers who need parallelism
  create separate GedcomReader instances. This aligns with
  the mechanical sympathy principle (no locking in the hot
  parsing path).
- Q: Should the parser enforce resource limits to protect
  against malicious or corrupt input? → A: Yes, configurable
  limits via GedcomReaderConfig with very generous defaults
  (high enough to never trigger on legitimate files). Covers
  max nesting depth and max line length.
- Q: What is explicitly out of scope for this feature?
  → A: Out of scope: DOM/tree building, GEDCOM
  writing/serialization, GEDZIP archive support. IN scope:
  structure and cardinality validation as optional
  warnings/errors. The parser SHOULD offer an opt-in
  validation layer that reports when structures appear in
  unexpected contexts or violate cardinality constraints.
...
The following capabilities are explicitly **out of scope** for
this feature and MUST NOT be implemented:

- **DOM / tree model construction**: The parser is a streaming
  SAX-like API. Building an in-memory tree is the consumer's
  responsibility.
- **GEDCOM writing / serialization**: This parser is read-only.
  Writing GEDCOM output is a separate future feature.
- **GEDZIP archive support**: The parser accepts an `InputStream`
  of GEDCOM text. Extracting from GEDZIP archives is the
  caller's responsibility.

The following capability is **in scope** as an opt-in layer:

- **Structure and cardinality validation**: The parser SHOULD
  provide an optional validation mode that reports warnings or
  errors when structures appear in unexpected contexts or
  violate cardinality constraints defined by the GEDCOM 7
  specification. This validation is NOT part of the core
  streaming parser but is an opt-in layer that consumers can
  enable via `GedcomReaderConfig`.
...
- **FR-040**: The parser MUST expose a callback-based API
  ...
  - `startDocument(GedcomHeaderInfo header)` -- fires before
    any record events. The GedcomHeaderInfo object carries
    standard HEAD metadata for quick access: GedcomVersion
    (major, minor, patch from GEDC.VERS), source system
    (SOUR tag and version), default language (LANG),
    and the SCHMA tag-to-URI map. The parser populates
    this by performing a preliminary pass over HEAD
    substructures. After startDocument fires, HEAD is then
    replayed as normal record events (startRecord through
    endRecord with all substructure events), giving
    consumers full visibility into HEAD content including
    any extension tags.
  - `endDocument()` -- after TRLR
  - `startRecord(int level, String xref, String tag)` --
    at each level-0 line including HEAD (except TRLR)
...
- **FR-094**: The parser MUST support configurable resource
  limits via `GedcomReaderConfig` to protect against
  malicious or corrupt input:
  - Maximum nesting depth (default: 1000). If a line's
    level exceeds this limit, a fatal error MUST fire.
  - Maximum line length in characters (default: 1,048,576
    i.e., 1 MB). If a single line exceeds this limit, a
    fatal error MUST fire.
  These defaults MUST be generous enough to never trigger
  on any legitimate GEDCOM file. Callers MAY lower the
  limits for untrusted input or raise them if needed.

#### Structure and Cardinality Validation (Opt-In)

- **FR-095**: The parser MUST support an optional validation
  layer, enabled via `GedcomReaderConfig`, that checks
  structures against GEDCOM 7 structure definitions.
- **FR-096**: When validation is enabled, the parser MUST report
  a warning when a structure (tag) appears in a context
  (superstructure) where it is not defined by the GEDCOM 7
  specification.
- **FR-097**: When validation is enabled, the parser MUST report
  a warning or error when a structure violates its cardinality
  constraint (e.g., a structure defined as `{0:1}` appears
  more than once under the same parent).
- **FR-098**: Extension tags (`_`-prefixed) MUST be exempt from
  structure validation (they are by definition outside the
  standard structure definitions).
- **FR-099**: Structure definitions MUST be compiled from the
  GEDCOM 7 specification's machine-readable files into Java
  source at build time, maintaining the zero-runtime-dependency
  principle.
...
- **NFR-012**: Each `GedcomReader` instance MUST be confined
  to a single thread. The parser MUST NOT use synchronization
  or locking internally. Callers who need to parse multiple
  files concurrently MUST create separate `GedcomReader`
  instances. This contract MUST be documented in Javadoc.
- **NFR-013**: The parser architecture MUST use a layered
...
- **GedcomReaderConfig**: Immutable configuration object
  ...
  strict vs lenient mode, max nesting depth (default 1000),
  max line length (default 1MB), structure validation
  enabled/disabled (default: disabled). Provides factory
  methods: `GedcomReaderConfig.gedcom7()` (default) and,
  in the future, `GedcomReaderConfig.gedcom555()`.
...
- **GedcomVersion**: Immutable value class representing a
  ...
  methods.
- **GedcomHeaderInfo**: Immutable value class carrying
  standard HEAD metadata for quick access: GedcomVersion,
  source system identifier and version (from HEAD.SOUR),
  default language (from HEAD.LANG), and the SCHMA
  tag-to-URI mapping (from HEAD.SCHMA.TAG). Passed to
  startDocument so consumers can inspect header metadata
  without manually tracking HEAD substructure events.
...
- **GedcomStructureDefinitions** (internal): Build-time-
  generated class holding structure context and cardinality
  rules compiled from the GEDCOM 7 specification's
  machine-readable files. Used by the opt-in validation
  layer. Not loaded unless validation is enabled.
...
Structure and cardinality validation is included as an opt-in
layer (see FR-095 through FR-099). It loads per-version
definitions compiled from the spec's machine-readable
TSV/YAML files into Java source at build time to maintain
